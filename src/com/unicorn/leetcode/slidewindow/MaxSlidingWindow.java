...
public class MaxSlidingWindow {
    ...
    public int[] maxSlidingWindow_best(int[] nums, int k) {
        ...
        for(int end =0 ;end<nums.length;end++){
            ...
            while(!queue.isEmpty() && nums[end] > queue.peekLast()){
                queue.pollLast();
            }
        ...
        public int[] maxSlidingWindow(int[] nums, int k) {
            Deque<Integer> queue = new ArrayDeque<>();// store index rather than value
            int start = 0;
            int[] result = new int[nums.length - k + 1];
            for(int end =0 ;end<nums.length;end++){
                while(!queue.isEmpty() && (end - queue.peekFirst() >= k)){
                    queue.pollFirst();
                }
    ...
    public int[] maxSlidingWindow1(int[] nums, int k) {
        ...
        for(int end = 0; end<nums.length;end++){
            while(queue.size()> 0 && nums[end] > queue.peek()){
                queue.poll();
            }
